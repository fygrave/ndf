...
my @fields = ('TimeGenerated','ComputerName','UserName','SourceNetworkAddress','WorkstationName','AuthenticationPackage');
my @fields_r = ('UserName','SourceNetworkAddress','WorkstationName');
##my @req_def = ('UserName','SourceNetworkAddress');

print $q->header(-charset=>'windows-1251'), $q->start_html("Search Logons"), $q->h1("Поиск по LogonActivity"), 
...
if ($q->param("find")){
	...
	for my $info (@items){
		...
		my $sth = $d->execSQL("SELECT TOP $top ".join(',',@fields).
			" FROM LOGONACTIVITY ".
			" WHERE $field = '$info' $required $str_conds ".
			" order by TimeGenerated desc "
		);
		print "<h3>Шаблон поиска: '$info'</h3><p><small> Доп условия: $required $str_conds</small></p><br>\n";
		$qx->printTableSth($sth,\@fields,
			##['','',"<A HREF=\"https://..yourdomaoin.ru/nomenu/addetails.aspx?name=USERLOGIN\">",
